...
全52基のうち、正しい被写体と再利用条件を確認できた51基にWikimedia Commonsの写真を使用する。五島の大瀬埼灯台は同名灯台との取り違えを避け、条件を満たす写真が見つかるまで図案表示を維持する。各詳細ページで作者、原版、ライセンスへのリンクを写真の直下に表示する。機械可読な完全一覧は `frontend/src/data/photo-sources.json` に保持し、登れる灯台16基を必須とする自動テストを行う。
...
上表は当初の16基を示す。追加分を含む51基の完全なクレジットは、各詳細ページと `frontend/src/data/photo-sources.json` に記録する。

Commonsの画像を最大1280pxのWebPへ変換し、サムネイルとともに `frontend/public/images/` に同梱して表示する。取得元URLはメタデータに保持し、`npm run sync:photos` で再取得できる。写真ライセンスは各写真にのみ適用され、アプリのソースコード全体のライセンスを変更しない。
