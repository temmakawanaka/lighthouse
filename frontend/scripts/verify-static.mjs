...
try {
  for (const record of [null, ...catalog]) {
    ...
    assert(html.includes(record ? record.name : "灯台を訪ね、スタンプを集める"), path);
  ...
  for (const [path, expected] of [["/map/", "全国の灯台マップ"], ["/stamps/", "灯台スタンプ帳"], ["/trip/", "灯台めぐりの旅程"], ["/my-lighthouses/", "マイ灯台"]]) {
    const response = await fetch(origin + path);
    assert.equal(response.status, 200, path);
  ...
  assert((await featured.text()).includes("宗谷岬灯台"));
  ...
  console.log(`Static export: homepage + ${detailDirectories.length} details, ${checkedAssets.size} assets, source dates, direct routes and genuine 404 passed.`);
} finally {
