...
describe("user backup", () => {
  ...
  it("exports and restores status, visit details, and trip data", () => {
    const status = { version: 2 as const, favorites: ["omaesaki"], visited: ["inubosaki"], visits: { inubosaki: { date: "2026-09-09", note: "晴れ" } } };
  ...
  it("rejects unrelated files and filters unknown records", () => {
    ...
    const value = JSON.stringify({ app: "lighthouse-field-guide", version: 1, exportedAt: "", status: { favorites: ["unknown"], visited: ["unknown"] }, trip: { stops: ["unknown"] } });
    expect(parseUserBackup(value, validSlugs).status.favorites).toEqual([]);
  });
});
