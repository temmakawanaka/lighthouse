...
export const LIGHTHOUSE_REGIONS = [
  ...
  { name: "東北", prefectures: ["青森県", "岩手県", "宮城県", "秋田県", "福島県"] },
...
export function getLighthouseRegion(prefecture: Lighthouse["prefecture"]) {
  return LIGHTHOUSE_REGIONS.find((region) => region.prefectures.some((value) => value === prefecture))?.name ?? "その他";
}
